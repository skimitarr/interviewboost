...
export default function MyQuestions() {
  ...
  const submitForm = async (e: FormEvent<HTMLFormElement>) => { // отправляем данные в гугл таблицу (и не только)
    ...
    В настоящее время вы проводите собеседование с кандидатом на должность ${storeProfession?.title}. Вам предоставлены конкретные данные об ответах кандидата на заданные вопросы:
    ...
    Категория это технологии, языки программирования и библиотеки - такие как (html, css, js и так далее)

    В своей оценке вы полагаетесь исключительно на предоставленные фактические оценки и комментарии. Вы не используете предположительные слова вроде "возможно", "вероятно", "скорее всего", "наверное".
