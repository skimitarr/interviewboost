...
export default function MyQuestions() {
  ...
  const submitForm = async (e: FormEvent<HTMLFormElement>) => { // отправляем данные в гугл таблицу (и не только)
    ...
    Комментарий каждого ответа (если есть)
    Средняя оценка кандидата по итогам всех ответов - ${averageMark}

    Твоя цель - проанализируй данные и на основе этих данных ты должен дать развернутый и обоснованный вывод о знаниях кандидата. В своем анализе ты опираешься строго на фактические оценки и комментарии. Ты не использую предположительные слова "возможно", "вероятно", “скорее всего”, “наверное”.
    Если средний балл кандидата после собеседование менее 3, ты категорически не рекомендуешь его к приему на работу. Если средний балл кандидата после собеседования менее 3 в какой то из категорий, ты не рекомендуешь его к приему на работу.
     В остальных случаях ты подробно описываю сильные и слабые стороны кандидата, чтобы помочь принять взвешенное решение о найме.
    Твоя цель - дать максимально конструктивный и объективный вывод.`];
