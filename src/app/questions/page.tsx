...
export default function MyQuestions() {
  ...
  const submitForm = async (e: FormEvent<HTMLFormElement>) => { // отправляем данные в гугл таблицу (и не только)
    ...
    copy[dataToGoogleSheets.length + 1] = [`Ты - FrontEnd разработчик, техлид нескольких команд, с большим опытом работы на разных коммерческих проектах. Ты проводишь собеседование кандидата на позицию ${storeProfession?.title}. Здесь тебе предоставлены данные собеседования: вопросы, оценки ответов на эти вопросы по пятибальной системе и комментарии к этим вопросам (если есть). Cредняя оценка за все собеседование - ${averageMark}. Пожалуйста, проанализируй данные и дай развернутый и обоснованный вывод о знаниях кандидата, опиши рекомендацию стоит ли брать кандидата на работу (ты должен строго оценивать, если оценки меньше 3 таких кандидатов не стоит рекомендовать), а также на что стоит обратить внимание кандидату, сильные и слабые стороны кандидата в знаниях исходя оценок.`];
